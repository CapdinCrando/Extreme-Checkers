...
std::vector<Move>* AIParallel::getAllRedJumps(GameBoard &board, boardpos_t pos)
{
	Move m;
	std::vector<Move> *jumps = new std::vector<Move>;
...
bool AIParallel::evalBoardResult(GameBoard &board, result_t& resultOut)
{
...
result_t AIParallel::evalBlackMove(GameBoard board, Move& move, depth_t depth)
{
	// Execute Move
...
result_t AIParallel::evalRedMove(GameBoard board, Move& move, depth_t depth)
{
	// Execute Move
	board.move(move.oldPos, move.newPos);
	if(MOVE_ISJUMP(move)) board.setSquareState(move.jumpPos, SQUARE_EMPTY);
